...
const run = async () => {
  ...
  if(menu.options !== "Import a file of locations ...")
  {
    ...
    if(response)
    {
      ...
      // Please check the documentation related to this function. 
      messages.returnTemp(response.name, response.sys.country, response.main.temp, tempFormat, response.main.humidity, response.weather[0].description);
    } else {
      ...
      try {
        ...
        fetchData.getMultipleWeather(citiesArr)
          .then(dataArr => {
            if(dataArr)
            {
              dataArr.map(data => messages.returnTemp(data.name, data.sys.country, data.main.temp, obj.temp, data.main.humidity, data.weather[0].description)
              );
            } else {
